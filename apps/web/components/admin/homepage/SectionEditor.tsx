...
export function SectionEditor({
  ...
  const isSingleStoryFeature = section.kind === "FEATURE";
  const slotLimit = isGoodNewsSection ? 6 : isSingleStoryFeature ? 1 : undefined;
  ...
  return (
    <section className="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-sm">
      ...
      {isSingleStoryFeature ? (
        <p className="mt-4 text-sm leading-7 text-zinc-600">
          This block is a single-story feature on the homepage — only slot 1 is shown to readers, so extra slots here won't appear.
        </p>
      ) : null}
