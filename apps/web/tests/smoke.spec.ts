...
test("newsletter signup and public contact routes accept submissions", async ({ page }) => {
  ...
  await page.getByRole("main").getByPlaceholder("Email address").fill("reader@example.com");
...
test("classified detail route accepts moderated enquiries", async ({ page }) => {
  await page.goto("/classifieds");
  const firstListing = page.getByRole("main").locator('a[href^="/classifieds/"]').first();
  await firstListing.click();

  await page.getByPlaceholder("Your name").fill("Buyer Example");
  await page.getByRole("main").getByPlaceholder("Email address").fill("buyer@example.com");
