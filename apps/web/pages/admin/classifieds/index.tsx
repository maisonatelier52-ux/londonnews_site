...
export const getServerSideProps: GetServerSideProps = async ({ req, res, query }) => {
  ...
  try {
    const [classifieds, totalCount, draftCount, reviewCount, publishedCount] = await Promise.all([
      ...
      prisma.classifiedListing.count({ where }),
      prisma.classifiedListing.count({ where: { ...where, status: "DRAFT" } }),
      prisma.classifiedListing.count({ where: { ...where, status: "IN_REVIEW" } }),
      prisma.classifiedListing.count({ where: { ...where, status: "APPROVED" } })
    ]);

    return {
      props: {
        classifieds: JSON.parse(JSON.stringify(classifieds)),
...
export default function ClassifiedsAdminIndexPage({
  classifieds,
  statusFilter,
  q,
  page,
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      ...
      params.set("page", "1");
      router.push(`/admin/classifieds?${params.toString()}`);
    }, 400);
  ...
  const filterHref = (filter: string) => {
    ...
    if (q) params.set("q", q);
    return params.toString() ? `/admin/classifieds?${params.toString()}` : "/admin/classifieds";
  };
      ...
      <section className="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 font-news text-5xl text-zinc-950">Listings and moderation</h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-700">
              Review incoming classifieds submissions, polish live listings, and manage public marketplace inventory from one desk.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <article className="rounded-[1.5rem] border border-zinc-200 bg-stone-50 p-4">
            <p className="text-[11px] uppercase tracking-[0.22em] text-zinc-500">Visible listings</p>
        ...
        <div className="mt-6 rounded-[1.5rem] border border-zinc-200 bg-stone-50 p-4">
          <label className="block">
            ...
            <input
              ...
              placeholder="Search titles, categories, locations, sellers, or slugs"
            />
          </label>
        </div>
        ...
        {dataUnavailable ? (
          <div className="mt-5 rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            Classified inventory is currently unavailable. Confirm the configured Postgres service is reachable.
          </div>
        ) : null}
      </section>
      ...
      <section className="rounded-[2rem] border border-zinc-200 bg-white p-4 shadow-sm">
              ...
              {classifieds.map((item: any) => (
                <tr key={item.id} className="transition hover:bg-stone-50">
                  <td className="px-4 py-4">
                    <Link href={`/admin/classifieds/${item.id}`} className="font-semibold text-zinc-950">
                      {item.title}
                    </Link>
                    <p className="mt-1 text-xs text-zinc-500">/{item.slug} • {item.category}</p>
                  ...
                  <td className="px-4 py-4 text-zinc-700">
                    ...
                    <div className="mt-1 text-xs text-zinc-500">{item.sellerEmail}</div>
                  </td>
        ...
        {classifieds.length === 0 ? (
          <div className="rounded-[1.5rem] border border-dashed border-zinc-300 bg-stone-50 p-8 text-center text-sm text-zinc-500">
            {q ? "No listings match that search yet." : "No listings match this filter yet."}
          </div>
        ) : (
          <Pagination
            page={page}
