...
export async function applyHomepageSnapshot(params: {
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      // Create every section (with its nested slots) concurrently instead of
      // awaiting them one at a time. Each `await` in a loop is a full
      // round-trip to the database; on a remote/serverless Postgres
      // connection those add up fast and can blow past Prisma's default
      // 5s interactive-transaction timeout once a homepage has more than a
      // couple of sections or slots. Firing them together keeps this well
      // under the (now also raised) timeout below.
      await Promise.all(
